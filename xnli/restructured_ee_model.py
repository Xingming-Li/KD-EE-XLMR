...
class XLMRWithEarlyExit(PreTrainedModel):
    def __init__(self, config, num_labels, is_token_classification=False, exit_layers=[4, 8], threshold=0.5):
    ...
    @classmethod
    def from_pretrained_custom(cls, model_path, num_labels, is_token_classification=False, exit_layers=[4, 8], threshold=0.5):
        # Load config
        config = AutoConfig.from_pretrained(model_path, output_hidden_states=True)

        # Init model with architecture only
        model = cls(config=config,
                    num_labels=num_labels,
                    is_token_classification=is_token_classification,
                    exit_layers=exit_layers,
                    threshold=threshold)

        # Load model weights
        state_dict = safe_load(f"{model_path}/model.safetensors")
        model.load_state_dict(state_dict, strict=False)

        # Optionally load exit heads separately
        try:
            exit_head_path = f"{model_path}/exit_heads.bin"
            exit_head_weights = torch.load(exit_head_path, map_location="cpu")
            model.exit_heads.load_state_dict(exit_head_weights)
        except FileNotFoundError:
            print("⚠️ exit_heads.bin not found. Assuming heads are randomly initialized.")
